...
private:
  ...
  void timerCallback()
  {
    ...
    double left_wing_angle  = (A_m_ - std::min(A_d_,A_m_))/2.0 * sine_val + alpha_m_ - alpha_d_;
    double right_wing_angle = - (A_m_ + std::max(A_d_,-A_m_))/2.0 * sine_val - alpha_d_ - alpha_d_;
    ...
    double left_flap_angle  = + theta_m_ + std::max(- abs(theta_m_), theta_d_/2);
    double right_flap_angle = + theta_m_ - std::min(+ abs(theta_m_), theta_d_/2);

    right_flap_angle = -right_flap_angle;
